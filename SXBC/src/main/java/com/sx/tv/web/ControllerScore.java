...
public class ControllerScore {
	...
	public String create(@Valid Score score, BindingResult bindingResult, Principal principal, Model uiModel, HttpServletRequest httpServletRequest) {
		...
		score.persist();
		return "redirect:/tvshows/generalInfo/" + URLStringUtil.encodeUrlPathSegment(score.getTvshow().getId().toString(), httpServletRequest);
	}

	@RequestMapping(value = "/create/{tvid}", params = "toCreate", produces = "text/html")
	public String createForm(@PathVariable("tvid") Long tvid, Model uiModel, Principal principal) {
		User u = User.findUsersByNameEquals(principal.getName()).getSingleResult();
		...
		uiModel.addAttribute("isCreate", isCreate);
		populateEditForm(uiModel, score);
		return "scores/createScore";
	}

	@RequestMapping(value = "/update/doUpdate", method = RequestMethod.PUT, produces = "text/html")
	public String doUpdate(@Valid Score score, BindingResult bindingResult, Model uiModel, HttpServletRequest httpServletRequest) {
		if (bindingResult.hasErrors()) {
			populateEditForm(uiModel, score);
