	...
	myDataTable.subscribe("dropdownChangeEvent", function(oArgs) {
		...
		if (opt == "none") {
			...
			jConfirm("真的要禁用《" + oRecord.getData("companyName") + "》吗?", '警告',
					function(r) {
						if (r) {
							window.location = "/tv/sys/doDisableEnableCompany.action?enableOperator=false&company.id="+ oRecord.getData("id");
						}
					});
		} else {
			elDropdown.selectedIndex = 0;
			...
			jConfirm('真的要启用《' + oRecord.getData("companyName") + '》吗?', '警告',
					function(r) {
						if (r) {
							window.location = "/tv/sys/doDisableEnableCompany.action?enableOperator=true&company.id="+ oRecord.getData("id");
						}
					});
		}
	});
